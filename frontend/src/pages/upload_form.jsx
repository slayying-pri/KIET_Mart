...
const Form = () => {
  ...
  const postData = async () => {
    try {
      const res = await fetch(`${backendURL}/add_data/`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      console.log("Raw response from /add_data:", text);
      ...
      if (res.status === 201) {
        ...
        window.alert(result.error || "Could not post your Data");
        return false;
      }
    } catch (err) {
      console.error("POST error:", err);
      window.alert("Server error while posting data");
      return false;
    }
  };
          ...
          <div className="container p-10 m-auto ">
            ...
            <div className="flex md:flex-row flex-col py-5">
              ...
              <Box sx={{ ml: 5 }}>
                ...
                <Rating
                  name="item_condition"
                  value={Number(data.item_condition)}
                  onChange={(event, newValue) =>
                    setData((prev) => ({
                      ...prev,
                      item_condition: newValue,
                    }))
                  }
                />
              </Box>
            ...
            <div className="my-5 text-center">
              <button
                type="submit"
                onClick={final}
                className="text-white bg-green-700 rounded-lg text-sm px-10 py-2.5"
              >
                Submit
              </button>
            </div>

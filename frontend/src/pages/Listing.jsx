...
//   const CallAboutPage = async () => {
//     console.log("backendURL value:", backendURL);
...
//     console.log("Raw response from /db:", text);

//     let object = [];
//     try {
//       object = JSON.parse(text); // try parsing JSON
//     } catch (err) {
...
//     // optional: check 200 instead of 201
//     if (res.status !== 200) {
//       alert(
//         "There seems to be some issue with your credentials or backend. We are working on it."
//       );
//     }
//   } catch (err) {
...
const CallAboutPage = async () => {
  ...
  // ✅ Add this log before fetch
  console.log("About to fetch from:", `${backendURL}/db`);
    ...
    const ress = Data.map((element) => {
      return {
        ...
        list: element.list.filter((currentVal) => {
          ...
          return currentVal.item_name.toLowerCase().includes(
  ...
  // Initial filter to remove empty lists
  const filterr = (object) => {
    const res = object.filter((currentValue) => currentValue.list.length !== 0);
    setData(res);
    setdat(res);
  };
                    ...
                    <div className="gap-6 lg:block">
                      ...
                      <div className="block my-1">
                        <input
                          ...
                          name="tag"
                          className="mr-2"
                          onChange={() => filtertags("Stationary")}
                        />
                        Stationary
                      </div>
                      <div className="block my-1">
                      ...
                      <div className="block my-1">
                        <input
                          ...
                          name="tag"
                          className="mr-2"
                          onChange={() => filtertags("Clothing_essentials")}
                        />
                        Clothing_essentials
                      </div>
                      <div className="block my-1">
        ...
        <div className="before:hidden items-center justify-items-center grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-2 md:gap-3 lg:gap-3 p-[1%] w-[100%] md:w-[90%] lg:w-[75%] sm:mx-0 md:mx-auto lg:mx-auto">
          {dat?.map((dataa) =>
            dataa.list?.map((listt) => (
              <div
                key={listt._id}
                className="bg-white shadow-grey-500 bg-opacity-40 rounded-md p-[1%] transition duration-500 border border-slate-200 hover:shadow-2xl hover:shadow-gray-300 m-auto mb-3"
              >
                <button
                  className="text-left"
                  onClick={() =>
                    navigate("/knowmore", { state: { item_id: listt._id } })
                  }
                >
                  <img
                    className="rounded-tl-3xl rounded-br-3xl object-cover w-[280px] h-[300px]"
                  ...
                  <div className="flex justify-between gap-[1%] text-gray-100">
                    <div className="flex">
                      <img className="object-contain w-8 m-1" src={image1} alt="" />
                      <div className="self-center">{listt.item_age} Months</div>
                    </div>
                    <div className="flex">
                      <img className="object-contain w-8 m-1" src={image2} alt="" />
                      <div className="self-center">
                        {listt.item_condition} Star Condition
                      </div>
                    </div>
